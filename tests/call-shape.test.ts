...
describe("classifyThrown nomeia a exceção que escapou do handler", () => {
  ...
  it("erro que NÓS escrevemos continua classificado pela mensagem", () => {
    expect(
      classifyThrown(new Error('Indicator "XX.INVENTADO" not found in the UIS catalog.'))
    ).toBe("nao_encontrado");
    expect(classifyThrown(new Error("The UIS API returned 503"))).toBe("fonte");
    expect(classifyThrown(new Error("Too many indicators (9) — maximum 5 per call."))).toBe(
      "contrato"
    );
  });

  it("lida com o que foi lançado sem ser Error", () => {
    expect(classifyThrown("The UIS API returned an empty response")).toBe("nao_encontrado");
